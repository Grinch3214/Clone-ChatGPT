<template>
	<aside ref="elementToDetectOutsideClick" class="px-5 py-8 bg-sidebar h-full absolute top-0 z-40 md:static w-[268px] md:w-full shadow-md transition-all" :class="[ chatGptStore.toggleEvent ? 'left-0' : '-left-full']">
		<button class="md:hidden fixed top-2.5 left-4 transition-opacity" :class="[chatGptStore.toggleEvent ? 'opacity-0' : 'opacity-100']" @click="chatGptStore.toggleEvent = true ">
			<Icon name="bx:menu-alt-left" size="26px" />
		</button>
		<button @click="chatGptStore.openNeewChat" class="mb-10 flex w-full gap-2 items-center text-sm hover:bg-secondary px-2 py-1 rounded-md transition-colors outline-none focus:shadow-border focus:shadow-white">
		...
		<button @click="modal = true" class="text-sm px-2 border border-white rounded-md w-full text-center py-2 mb-4 hover:bg-secondary transition-colors outline-none focus:shadow-border focus:shadow-white">Clear history</button>
		...
		<div class="overflow-x-hidden overflow-y-auto h-[calc(100vh-190px)] relative before:content-block before:absolute before:right-0 before:top-0 before:z-10 before:bottom-0 before:w-8 before:bg-gradient-to-l before:from-sidebar before:pointer-events-none">
			<AppList v-for="(list, index) in reverseDatamessage" :key="index" :prompt="list" />
		</div>

		<AppModal v-show="modal" title="Do you want to clear all history?" @close="modal = false">
...
<script setup lang="ts">
	...
	const deleteAllMessages = () => {
		chatGptStore.deleteAlldataMessages()
		modal.value = false
	}

	const reverseDatamessage = computed(() => {
		return chatGptStore.dataMessages
	})
